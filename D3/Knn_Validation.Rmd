...
Train <- cbind(dist[Index,],y[Index])
Test <- cbind(dist[-Index,],y[-Index])
...
for(i in k){
  set.seed(123)
  folds <- createFolds(Train[,ncol(Train)], k=f,list=TRUE) # hay la funcion createfolds que te crea las capas del crossval
  ...
  for(j in 1:f){
    ...
    model_cv <- knn(train=Train_cross[,-9], test=Test_cross[,-9], cl = Train_cross[,ncol(Train_cross)], k=i) 
    # Usamos knn para entrenar el modelo
    # CAMBIAR -9 si añades las categoricas 
    
    conff_cv <- table(Real = Test_cross[,ncol(Test_cross)], Predicted = model_cv) # Creamos la matriz de confusion y calculamos el accuracy
...
```{r}
model <- knn(train = Train[,-9], test=Test[,-9], cl=Train[,ncol(Train)], k=k_opt)
```
