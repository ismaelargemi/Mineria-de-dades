...
p[[1]]
```
 Como primera variable tenemos el sexo de los individuos, vemos que el 62.0% son mujeres y solo el 32.0% son hombres.
 
```{r, echo = F, fig.cap= "Pie Chart NAME INCOME TYPE",fig.show='hold',out.width="75%",out.height="75%"}
p[[2]]
```

En esta gráfica podemos ver que más del 55% de los individuos recibe ingresos porque trabaja,  un 22.2% son comerciales y el otro 21.4% se dividen entre jubilados,trabajadores públicos y empresarios. 

```{r, echo = F, fig.cap= "Pie Chart NAME EDUCATION TYPE",fig.show='asis',out.width="75%",out.height="75%"}
p[[3]]
```
Por lo que se refiere a estudios se observa que aproximadamente el 75% de los individuos ha terminado el Bachillerato. 

```{r, echo = F, fig.cap= "Pie Chart NAME FAMILY STATUS",fig.show='asis',out.width="75%",out.height="75%"}
p[[4]]
```
En la variable 'name family status', se observa que la mayoría de personas están casadas por la iglesia(61.9%), otras están casadas por matrimonio civil(10.9%) y otras están solteras(16.0%). Una pequeña parte son viudas(4.82%)	 y divorciadas(6.40%)  

```{r, echo = F, fig.cap= "Distribucion OCCUPATION TYPE",fig.show='asis', fig.height=10, fig.width=10}
p[[5]]
```
Por lo que se refiere a la variable 'Occupation type' se observa que la mayoría son NA's, casi un 30%, en segundo lugar tenemos los trabajadores que son casi un 20% y en tercer lugar los vendedores con más de un 10%.

```{r, echo = F, fig.cap= "Distribución ORGANIZATION TYPE",fig.show='asis', fig.height=10, fig.width=10}
p[[6]]
```

En esta gráfica podemos ver que otra vez los Na's son una de las variables con más peso (15.2%), junto con los trabajadores del mundo del Business de tipo 3 (23.3%) y los autónomos (14.1%). 
```{r, echo = F, fig.cap = "Pie chart REGION RATING CLIENT", fig.show='asis',out.height="75%",out.width="75%"}
p[[7]]
```
 En la variable 'Ragion rating client', se observa que la mayoría de personas son d ela región 2 (72.82%), otras pocas son de la región 3(18.5%) y otras son de la región 1 (8.6%)
 

```{r, echo = F, fig.cap = "Pie Chart TARGET", fig.show='hold', out.height="75%", out.width="75%"}
p[[8]]
```
Este gráfico se reflejan los datos de la variable "Target". Se observa que el 42.7% de los clientes son clientes con dificultades de pago: él/ella tuvo pagos atrasados de más de X días en al menos una de las primeras Y cuotas del préstamo en nuestra muestra. El 57.3% restante son todos los demás casos.
...
print(p)
```
Al realizar una división por sectores podemos ver que el sector del Business es donde la mayoría de individuos trabajan. También el comercio y el sector de la industria son bastante comunes.
