...
lda_cv = function(df_train, k){
  ...
  for(i in 1:k){
    ...
    ind = c(1+((i-1)*(nrow(df_train)/k)),(nrow(df_train)/k)*i)
    test = df_train[c(ind[1]:ind[2]),]
    train_cv = df_train[-c(ind[1]:ind[2]),]
...
predicciones = factor(ifelse(prob>0.5, 1,0))
...
def = qda(reformulate(x,y), data = train)
def$scaling = qda_cv(train, 4)

valores = predict(def, validation)
