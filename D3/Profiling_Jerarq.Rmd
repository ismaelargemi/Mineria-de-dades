...
A partir del gráfico, se observa que en los ingresos totales el cluster 1 se caracteriza por tener el menor número de ingresos  y el clúster 3 el que mayor los tiene. No obstante, no hay diferencias muy grandes entre ellos debido a que es el logaritmo de estos ingresos y la interpretacion queda afectada.
...
El clúster 1 se caracteriza por ser el grupo con individuos de más edad, en otras palabras, es el clúster con los individuos más grandes.
...
El primer grupo es el de los pensionistas y el quintó es el de los commerciantes asociantes.
...
En el clúster 4 pertenece la gente no casada o casada civilmente.
...
Respecto al grupo uno no se puede saber a que tipo de organización pertenecen porque no trabajan, son los pensionistas. El resto sigue una distribución muy parecida aunque el grupo uno parece estar más dedicado a la educación que los otros. (conclusión dudosa)
```{r}
...
Clúster 1: Se caracteriza por individuos con pocos familiares, menor número de ingresos, son los de mayor edad, mujeres, pensionistas y de los que más tardan de devolver el préstamo.

Clúster 2: Como en el uno, son mujeres, proporcionalmente con más "mid skill workers", con una alta cantidad de familiares, mayor cantidad de importe del préstamo y son los que peor capacidad de devolver el préstamo tienen, es decir, los que más se demoran en devolverlo.

Clúster 3: Es el grupo con mayor número de ingresos, son hombres y con más cantidad de familiares.

Clúster 4: En este clúster están los individuos con pocos familiares, menor importe de crédito por préstamo y ratio de anualidad más grande. Se caracteriza por estar compuesto en la misma proporción tanto de hombres como de mujeres. Los individuos están solteros o casados civilmente y tienen menor cantidad de importe en el préstamo. Finalmente, son los más rápidos en devolver el crédito.

Clúster 5: Este último grupo está configurado con los individuos que tienen los coches más nuevos, mayor número de crédito por préstamo, son mujeres, en concreto "commercial associates".


Comparación Profiling K-means y Jerárquico

El número de clústers se diferencia en los dos métodos.

Vemos como en el clústering jerárquico se obtienen perfiles de cada grupo muy específicos y caracterizables, a diferencia de los perfiles obtenidos en la metodología k-means.

Por último, en la metodología k-means se observa como solo se ven explicados los grupos por algunas variables numéricas y ninguna categórica, debido a que es un método que solo trabaja con variables numéricas. Por esta razón, la variabilidad explicada por las variables categóricas no se ve reflejada en el profiling de k-means y obtenemos unos perfiles muy poco caracterizados.
