...
En este apartado se aplicarán diferentes algoritmos de clasificación, en concreto el k-means y el Jerárquico. Todo ello, con el objetivo de determinar a qué grupo pertenece cada observación, para poder realizar un Profiling en el que se terminará etiquetando a cada uno de estos grupos con sus características más relevantes.

## K-means

Primero se cargan los datos y se separan las variables numéricas:
...
Por lo que sigue, como existen muchos otros criterios para la selección de la k óptima, se usará la función NbClust, que permite aplicar una cantidad de 26 criterios para la selección de k, de esta manera se sabrá con mayor seguridad cuál es el óptimo real.
...
Ahora bien, como se puede ver en el gráfico anterior, no se distinguen muy bien las tres clases, ya que están unas encima de las otras. Esto puede ser consecuencia de que la clasificación se ha hecho considerando solamente las variables numéricas, es por eso que es necesario realizar un clustering jerárquico.

## Clustering jerárquico

En primera instancia, para realizar el clustering jerárquico se debe hacer primeramente un dendograma con el método de Ward con la distancia de $$Gower^2$$. En el k-means solo se puede trabajar con las variables numéricas y en la base de datos hay variables tanto numéricas como cualitativas. La distancia de $$Gower^2$$ nos permitirá calcular las distancias tanto de las variables numéricas como de las categóricas.
...
Con el dendograma anterior, se confirma que el mejor corte (después de k = 2) es k = 3 y k = 5.
...
Finalmente, se guardan los datos preprocesadas con los clústeres hechos. 
