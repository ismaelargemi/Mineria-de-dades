...
El gráfico indica que la tasa de error para la 10-fold-cross-validation se estabiliza cuando el número de nodos hoja alcanza aproximadamente 9. Para determinar el tamaño ideal del árbol, la línea punteada representa el mínimo de la curva con un error estándar adicional de 1, que es una práctica común en la poda de árboles de decisión, donde se selecciona el árbol más pequeño dentro de 1 SE del mínimo.
...
### Validación del modelo con del ÁRBOL DE DECISION 1
...
```
En el gráfico se observa como el primer valor del complexity parameter es el que reporta un mayor accuracy tanto para el conjunto de datos de entrenamiento. No obstante, no únicamente buscamos el hiperparámetro que nos aporta un mayor accuracy, sino que también nos interesa encontrar un cp con el que además de maximizar el accuracy, evitemos overffiting (sobreajuste del modelo). Así pues, observamos como el segundo valor del complexity parameter nos da un valor que no se aleja mucho del accuracy mayor, y además nos evita en una gran manera overfitting. Por lo tanto, el cop óptimo para nuestro árbol de decisión final es `r model2$results$cp[2]`
...
En este caso, la precisión ha sido del `r round(MC$overall["Accuracy"],4)`%, lo que indica que el algoritmo ha predicho correctamente el `r round(MC$overall["Accuracy"]*100, 4)`% de los individuos de Test. 

La "Sensitivity" mide la proporción de individuos de TARGET=0 que han sido clasificados correctamente, que en este caso ha sido de `r round(MC$byClass["Sensitivity"]*100,4)`.

Por otro lado, la "Specificity" mide la proporción de individuos de TARGET=1 que han sido clasificados correctamente, que ha dado un valor muy elevado, de`r round(MC$byClass["Specificity"]*100,4)`.

Si observamos otras métricas disponibles, apreciaremos una tasa de valores positivos predichos de `r round(MC$byClass["Pos Pred Value"]*100,4)`% y una tasa de valores negativos predichos de `r round(MC$byClass["Neg Pred Value"]*100,4)`%. Este hecho implica que al predecir una clase, la probabilidad de que ésta sea clasificada correctamente es de entorno al 77%. Por último, podemos apreciar que el valor del F-score es de `r 2/((1/MC$overall["Accuracy"]) + (1/MC$byClass["Specificity"]))`, métrica perjudicada por el bajo valor de la sensibilidad. \newline
...
rpart.plot(tree$finalModel, box.palette = "auto", shadow.col = "gray", nn = TRUE)

```
El árbol de decisión generado se inicia evaluando la edad del solicitante. Si la edad es mayor o igual a 57 años, el modelo tiende a clasificar al individuo como "No Moroso" con una probabilidad del 81.8%. Este primer nivel de decisión sugiere que la edad es un factor determinante en la predicción de la no morosidad.

Por otro lado, dentro de la categoria de clientes más jovenes, el árbol se ramifica según el ratio anualidad/crédito (RATIO_ANNITY_CREDIT). Aquellos con un ratio inferior a 0.0295 presentan una alta probabilidad de no ser morosos (86.21%), sugiriendo que clientes con cargas de anualidad más bajas en comparación con su crédito son menos propensos a tener dificultades de pago. 
...
Clientes casados y con más de 2 miembros familiares tienden a tener una probabilidad del 61.25% de no ser morosos. En casos específicos, como aquellos con menos de 2 miembros familiares y educación incompleta, la probabilidad de no morosidad alcanza el 72.25%.
...
Así pues, como podemos observar, en orden de importancia, la variable "Edad" es la más relevante a la hora de clasificar a los clientes en morosos y no morosos, sugiriendo que en el 70% de las veces en las que se cree un árbol de decisión, saldrá esta variable como la principal.  
