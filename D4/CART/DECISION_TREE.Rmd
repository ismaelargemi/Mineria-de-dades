...
### Validación del modelo con del ARBOL DE DECISION 1
...
La anterior salida nos muestra la matriz de confusión junto con diversos estadísticos que tratan de explicar como de bien o mal ha predicho el algoritmo de CART. Así pues, como se observa que las medidas de validación son aproximadamente las mismas tanto para Train como para Test, reafirmamos que no hay un sobreajuste en el modelo (como ya se había dicho anteriormente). 
...
La "Sensitivity" mide la proporción de individuos de TARGET=0 que han sido clasificados correctamente, que en este caso ha sido de `r MC$byClass["Sensitivity"]*100`.
...
Por otro lado, si la edad es menor a 62 años, el árbol se ramifica, y la siguiente variable considerada es el importe de crédito del préstamo (log_AMT_CREDIT). Si el monto del crédito es mayor o igual a 13, la clasificación es nuevamente "No Moroso" (NM). Esto indica que para los solicitantes más jóvenes con montos de crédito más altos, el modelo tiende a prever que no serán morosos, con una tasa de acierto del 30%.

En caso de que el monto del crédito sea menor a 13, el árbol examina la edad del automóvil propio (OWN_CAR_AGE), dividiéndose en dos caminos. Si la edad del automóvil es menor a 9 años, el modelo clasifica como "No Moroso" (NM). Sin embargo, si la edad del automóvil es mayor o igual a 9 años, se procede a una serie de condiciones adicionales basadas en otras variables como RATIO_CREDIT_INCOME, RATIO_ANNUITY_CREDIT, DTI_RATIO, y otras.
