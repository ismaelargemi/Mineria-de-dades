...
La profundidad de árbol se ha escogido que sea 7 debido a que como se puede ver en el gráfico, la accuracy parece estabilizarse a partir de esa profundidad, por lo que los siguiente niveles del árbol no aportarian información extra que ayudase a mejorar la precisión de este.
...
plot(x=1:20,xgb_model$evaluation_log$train_error, type="l")
points(x=1:20,xgb_model$evaluation_log$train_error, cex=0.5, col="red",pch=19)
```
...
xgb.plot.tree(model=xgbTree$finalModel, trees = 99)
```
